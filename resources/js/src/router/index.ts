...
const routes: RouteRecordRaw[] = [
    ...
    // verify-email without params: shown right after signup
    {
        path: '/auth/verify-email',
        name: 'verify-email',
        component: () => import(/* webpackChunkName: "auth-verify-email" */ '../views/auth/verify-email.vue'),
        meta: { layout: 'auth' },
    },
    // verify-email with id/hash: user clicked the link in their email
    {
    ...
    },
    {
        ...
        name: 'admin-login',
        component: () => import(/* webpackChunkName: "auth-admin-login" */ '../views/auth/admin-login.vue'),
        meta: { layout: 'auth', guestOnly: true },
    },
    {
...
router.beforeEach(async (to, from, next) => {
    ...
    // Set layout
    appStore.setMainLayout(to?.meta?.layout === 'auth' ? 'auth' : 'app');
    ...
    if (to.name === 'verify-email' || to.name === 'verify-email-link') {
        return next();
    }

    // ── Guest-only pages (login / signup / admin-login) ──
    if (isGuestOnly) {
        if (isAuthenticated) {
            // Already logged in: send to the correct dashboard
            return next({ name: isAdmin ? 'admin-home' : 'home' });
        }
    ...
    if (requiresAuth && !isAuthenticated) {
        // Unauthenticated: send to the right login portal based on destination
    ...
    if (to.name === 'home' && isAuthenticated && isAdmin) {
